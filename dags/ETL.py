...
def carregar_dados_estagio(dados_chunk):
    ...
    try:
        for arquivo in arquivos:
            ...
            for chunk in pd.read_fwf(arquivo, widths=separar_campos, header=0, chunksize=tamanho_chunk):
                ...
                carregar_dados_estagio(chunk.to_dict(orient='records'))
...
def transformar_e_enriquecer_dados():
    conn = obter_conexao_db()
    ...
    try:
        ...
        with conn.cursor() as cursor:
            cursor.execute(""" 
            INSERT INTO dim_ativos (ativo, nome_empresa)
...
with DAG('cotahist_etl', schedule_interval='@daily', start_date=datetime(2022, 1, 1), catchup=False) as dag:
    tarefa_extrair = PythonOperator(task_id='extrair', python_callable=extrair_dados)
    tarefa_transformar_e_enriquecer = PythonOperator(task_id='transformar_e_enriquecer', python_callable=transformar_e_enriquecer_dados)
